...
A systematic analysis of the structure of neural systems in different organisms points out that from the small nematode worm \emph{Caenorhabditis Elegans} to the most complex human brain, modularity plays a prominent role.
...
Further, meta-analytic studies of cortical networks for both the cat~\cite{scannel1995} and macaque~\cite{felleman1991} suggest that such networks are modular with a limited number of (typically four) prominent clusters corresponding to the main functional division of the cortex, visual, auditory, frontolimbic and somatosensory.

Relying upon such findings is the additional evidence for such networks as consistently identified by multivariate methods such as Independent Component Analysis (ICA)~\cite{beckmann2012,calhoun2001}, Probabilistic Independent Component Analysis (PICA)~\cite{beckmann2004,beckmann2005}, general linear models , clustering~\cite{cordes2002,lee2012}


\begin{figure}[!ht]
\centering
\includegraphics[width=1.0\textwidth]{images/vandenheuvel_networks.pdf}
\caption{Resting state networks}
