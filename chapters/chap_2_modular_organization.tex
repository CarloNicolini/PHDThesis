...
Indeed, although simple in many senses, Modularity optimization hided a problem that heavily limits its use in real world networks.
...
Due to its intrinsic stochastic nature, the Louvain method returns different near-optimal partitions for every run and must be typically run thousands of times in order to identify the best solution.
...
From it, it's possible to obtain an equivalent linear algebra formulation of Modularity that reads:
