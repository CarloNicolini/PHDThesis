...
None of the previously reported generative model account explicitly for the modular structure. The modules often observed in the ER model are due to statistical fluctuations. The BA model, too, does not take into account a block structure, when generating a power-law network. To overcome this limitations, the most commonly used generative model for random modular networks is called the stochastic block model (SBM)~\cite{holland1983}, which generalizes the Erd\H{o}s-Rényi random graph by giving each pair of nodes a connection probability depending only on their module affiliation.
...
In the case the matrix $\mathbf{B}$ takes only two different values, the simplest variant of the stochastic block model dubbed as the \emph{planted partition model}, each pair of nodes in the same block are connected with probability $B_{ij}=p_{in}$ and pairs of nodes in different modules are linked with probability $p_{out}$, as shown in Figure~\ref{fig:planted_peixoto}, or in other terms the module linking probability matrix is $\mathbf{B}= p_{in}\delta(\sigma_i,\sigma_j) + p_{in}\delta(\sigma_i,\sigma_j) $
...
While, on the theoretical side these properties are of great help when performing analytical calculations, they also make it badly suited for real world applications.
These features are unrealistic as complex networks are known to be characterized by heterogeneous degree distributions and community sizes.

To overcome the unrealistic settings of the planted partition model, the Lancichinetti-Fortunato-Radicchi (LFR) model first described in~\cite{lancichinetti2008} modified the planted partition model to exhibit powerlaw distribution of both node degrees and community size, as observed in real networks.
The LFR model has been used primarily as benchmark for evaluating the effectiveness of different community detection methods. It was further extended to be able to generate weighted networks with overlapping modules in~\cite{Lancichinetti2009a}. 

The LFR model has a richer variety of parameters.
\begin{table}[htb!]\centering
\begin{tabular}{lr}
\hline 
$\left<k\right>$  & Desidered average degree.\\
$\tau_d$ & Coefficient for the power-law distribution of nodes degrees. \\
$\tau_c$ & Coefficient for the power-law distribution of community size. \\
$\mu_t$ & Topological mixing coefficient.\\
$\mu_w$ & Weights mixing coefficient.\\
$\beta$ & Exponential parameter for strenghts distribution.\\
$\min_c$ & Minimum number of nodes in each community.\\
$\max_c$ & Maximum number of nodes in each community.\\

\end{tabular}
\caption{Meaning of LFR parameters.}
\end{table}
