...
\end{equation}
Modularity takes values in the range $[-0.5,1]$. 

A good partition should have $Q^N$ values close to unity, identifying groups with many more internal connections than expected at random; in contrast, a bad partition with $Q^N$ closer to zero should identify groups with no more internal connections than we expect at random.
...
The wide use of Modularity has, on one hand, opened many researchers to gain interest in complex networks, with some interesting results in sociology\todo{citare}, bioinformatics\todo{citare} and ICT\todo{citare}, on the other hand offered a fallacious view on community detection as a big problem was lurking in the very same definition of Modularity.

A seminal article by Fortunato and Barthelemy~\cite{fortunato2007} did a thoroughly analysis of Modularity, showing the inability of Modularity to correctly detected communities smaller than a certain scale, determined by the square root of the number of edges. They dubbed this general phenomenon \emph{resolution limit}.

To show the onset of the resolution limit, let us consider a toy network, $G=(V,E)$ that is composed of three subnetworks.
The first subnetwork, a subgraph $G_0$ with $n_0$ nodes, $m_0$ edges is connected to cliques $G_1$ and $G_2$ by $m_{01}$ and $m_{02}$ links respectively. The two cliques are also connected by a number of $m_{12}$ links as shown in Figure~\ref{fig:resolution_limit_fortunato}.

While $G_1$ and $G_2$, are complete subgraphs and are expected to be modules by construction, $G_0$ may consist of many communities. Maximum Modularity partitions then, should identify $G_1$ and $G_2$ as communities, independently from $G_0$. To be more specific, the partition where the two cliques are separated consists in three modules and its Modularity value is denoted by $Q_{split}$. The partition instead where the two cliques are merged into a single community is denoted by $Q_{merged}$. Because Modularity is a sum over the modules,
and the module $G_0$ with Modularity $Q_0$ is the same in both partitions, we are interested in the difference $\Delta Q = Q_{merge} - Q_{split}$ that results:

\begin{align} 
\Delta Q = \left[ \right]
\end{align}
\todo{CONTINUARE DA QUI}
