...
In the previous section we introduced the resolution limit and noticed that the differences in Modularity when evaluating wheter to split or to merge two modules may be very small.
Indeed, even in the case where it would not be beneficial for Modularity to merge two modules, i.e $\Delta Q <0$, this difference can be made arbitrarily close to zero. Because the number of possible different partitions of a graph with $n$ nodes (the Bell number $B_n$) grows faster than exponentially in $n$~\cite{stanley1997}, then a combinatorially large number of operations may lead to solutions that are radically different in terms of partition distance, but only slightly suboptimal in terms of modularity, with values very close to the global maximum.
...
This consideration explains the observation that nearly-optimal solution tend to group into high-modularity plateaus, although thay may differ substantially~\cite{good2009}. This phenomenon, dubbed as \emph{degeneracy} affects Newman's Modularity and probably many other spin-glass based quality functions.
As a matter of example, in the ring of clique graph, penalty in Modularity for merging a pair of adjacent cliques in the ring of cliques graph~\ref{fig:traag_ring_of_cliques} is given by:
...
In the large $r$ limit, the difference in Eq.~\ref{eq:modularity_difference_ring_clique} tends to a very small but negative value, indicating that a solution where the cliques are merged, may have a Modularity very close to the optimum. Indeed, already for $r=20$ cliques, $\Delta Q \approx 5\times 10^{-3}$, making all the suboptimal partitions where cliques are merged, very close to the optimal solution.

This powerful argument against the improper use of Modularity, can be made also more visually appealing. We sampled the configuration space of partitions in a ring of clique network through a Montecarlo procedure and annotated its corresponding values of quality function for each partition.
