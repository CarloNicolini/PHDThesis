...
Nonetheless, the notion that brain function can be reduced to the single operations of its constituents is as flawed as its complementary idea that cognition can be understood without making reference to its biological substrate. Instead, what is being increasingly recognized is that most complex systems are irreducibly complex and that only an approach that contemplate the multiple interactions of simple subsystem, is able to account for their observed features.
...
In this sense, the methods and algorithms offered by complex network theory are the most powerful and have already been adopted in clinical contexts, where they proved able to distinguish differences in the  brain connectivity, driving clinicians to quantitative diagnoses of diseases and eventually leading to a cure.

The study of brain connectivity has already made possible new advances in areas of neuroscience. Specifically the analysis of the neural network architecture is based on the idea that the brain can be thought as a complex system at different spatial and time scales.
Casting some of the problems that are typical of neuroscientific investigation into the language of network theory has shed light on a large number of problem concerning brain function.
%%%%%%
\bigbreak
...
Structural and functional brain networks can be explored using graph theory through four steps.
\begin{enumerate}
