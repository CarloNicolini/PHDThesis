...
The most commonly used generative model for random modular networks is called the stochastic block model (SBM), which generalizes the Erd\H{o}s-Rényi random graph by giving each pair of nodes a connection probability depending on what groups they belong to.

In simplest variant of the stochastic block model, i.e. the \emph{planted partition model}, to each node pair is assigned a probability of linking $\Pr_{ij}$ that only depends on the module each node belongs to.
In particular, one sets the number of modules $|C|$ (or blocks) in advance and define an equal number of nodes in every module.
Nodes in the same block are then connected with probability $p_{in}$ and nodes in different modules with probability $p_{out}$, as shown in Figure~\ref{fig:planted_peixoto}.
...
Although largely used, the planted-partition model has several drawbacks. Indeed, each node has the same expected degree and all communities have the same size. While, on the theoretical side these properties help when analytically studying the property of the model, they also make it badly suited for real world applications. These features are unrealistic as complex networks are known to be characterized by heterogeneous degree distributions and community sizes.

To overcome the unrealistic settings of the planted partition model, the Lancichinetti-Fortunato-Radicchi (LFR) model first described in~\cite{lancichinetti2008} modified the planted partition model to include powerlaw degree distribution and community size, as observed in real networks. The model was further extended to be able to generate weighted networks with overlapping modules in~\cite{Lancichinetti2009a}. 
