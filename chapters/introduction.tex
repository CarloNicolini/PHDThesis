...
Generalizing the concepts of random networks models, one can define every simple graph on $n$ nodes as the output of a random process where each edge $(i,j) $ is sampled with a probability $\Pr(i,j,{\boldsymbol \theta})$ with $\boldsymbol \theta$ is a vector of hyperparameters. In the simplest case of the ER graph, the generative model is an uniform probability distribution as every pair of nodes is linked by an edge with constant probability:
\begin{equation}
\Pr(i,j) = p.
\end{equation}
...
In the Watts-Strogatz model a graph with groups of densely connected nodes is generated as follows:
\begin{enumerate}
...
\section{Community detection in networks}\label{sec:communitydetectioninnetworks}
...
\Pr \left ( a_{ij} | \{ k \} \right) = \Pr(k_i | \{ k \})\Pr(k_j | \{ k \})=\frac{k_i k(j)}{2m}
\end{equation}\todo{fixare formula non precisa}
...
Consider for example the set of matchings on six edges that form a triangle graph as illustrated in Figure~\ref{fig:configuration_model_stubs}. The configuration model chooses each distinct edge stubs labeling with equal probability. However, not only the first eight distinct reshuffled labelings are possible under configuration model (Fig.\ref{fig:reshuffle_simple_graphs}) but three other distinct matchings, which produce non-simple networks (Fig.\ref{fig:reshuffle_loopy_multigraphs}). In practice though, the fraction of edges involved in either self-loops or multi-edges is vanishingly small in the large $n$ limit, and thus we may generally ignore them without much impact. Hence, the estimate of the probability of rewiring in Eq.~\ref{eq:configuration_model_probability} is only valid in large, sufficiently sparse graphs with a sufficiently bounded degree sequence.
...
\todo{migliorare questa parte usando il libro di newman}
\begin{figure}[htb]
\centering
